...
class Dav_PhyOperation
{
    ...
    public static function removePath($path)
    {
        try {
            ...
            if ($resources == '*') {
                $upperResource = dirname($path);
                if (!file_exists($upperResource)) {
                    return true;
                }
                $resources = scandir($upperResource);
                $resources = array_diff($resources, ['.', '..']);
                if (empty($resources)) {
                    return true;
                }
                foreach ($resources as $resource) {
                    $resource = $upperResource . DIRECTORY_SEPARATOR . $resource;
