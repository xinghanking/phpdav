 ...
 * Class Method_Lock
 */
class Method_Lock extends Dav_Method
{
    ...
    {
        $lockInfo = Dav_Utils::getObjElements(self::BODY_ROOT);
        if (empty($lockInfo)) {
            throw new Exception(Dav_Status::$Msg[422], 422);
        }
        $timeout = empty($_REQUEST['HEADERS']['Timeout']) ? 'Infinite, Second-3600' : $_REQUEST['HEADERS']['Timeout'];
        ...
        $this->arrInput['locktoken'] = Dav_Utils::getLockToken();
        $lockscopeInfo = Dav_Utils::getObjElements(self::BODY_ROOT . '/lockscope');
        if (0 == $lockscopeInfo->length) {
            throw new Exception(Dav_Status::$Msg[422], 422);
        }
        $this->arrInput['lockscope'] = $lockscopeInfo->item(0)->childNodes->item(0)->localName;
        $locktypeInfo = Dav_Utils::getObjElements(self::BODY_ROOT . '/locktype');
        if (0 == $locktypeInfo->length) {
            throw new Exception(Dav_Status::$Msg[422], 422);
        }
        $this->arrInput['locktype'] = $locktypeInfo->item(0)->childNodes->item(0)->localName;
        $this->arrInput['owner'] = Dav_Utils::getElementList(self::BODY_ROOT . '/owner/href');
